...
const { OperationList, Operation, Machining, Material, Process, WorkCenter, MeasuringFrequency,dimension, Instrument, PlanAbstract, Drawing, DrawingType,ProcessCharacteristics,ProductCharacteristics,IncomingSource } = require('./../models')
...
router.get('/data/dimension', (req, res) => {
    return new Promise((resolve, reject) => {
...
router.get('/data/operationdetails', (req, res) => {
    return new Promise((resolve, reject) => {
        Operation.findAll().then(function (result) {
            sendSuccess(res, result);
        }).catch(function (err) {
            sendError(res, err);
        });
