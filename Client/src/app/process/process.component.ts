...
import { RunchartComponent } from '../runchart/runchart.component';
...
import { DimensionDialogComponent } from '../master/dimension/dimension-dialog/dimension-dialog.component';
import { DimensionService } from '../master/masterservice/dimension.service';
...
export class ProcessComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.islog && this.isENGG || this.isET) {
      this.displayedColumns = ['id', 'description', 'baloonNo', 'specification', 'tolFrom', 'tolTo','instrument','dimension','measuringFrequency', 'firstPartInspection','periodicInspection','ctq','pdi','cfir', 'delete'];
    }
    else if (this.islog && this.isTT) {
      this.displayedColumns = ['id', 'description', 'baloonNo', 'specification', 'tolFrom', 'tolTo', 'instrument','dimension', 'measuringFrequency'];
    }
  ...
  getDimension()
  {
    ...
    this._process.getDimension().subscribe((res: any) => {
      if (res.success) {
  ...
  addProcess() {
    ...
    let tempdata2 = this.dimensionList;
   ...
   for (var j in tempdata2) {
      if (tempdata2[j].name == step1.dimension) {
        step1.insId = tempdata2[j].id;
      }
    }  
